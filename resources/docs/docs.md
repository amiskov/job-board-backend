...
The `home-routes` handler in the `job-board-server.routes.home` namespace
...
The `home-page` function will in turn call the `job-board-server.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `job-board-server.handler` namespace:
...
Request middleware functions are located under the `job-board-server.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `job-board-server.dev-middleware` namespace found in
